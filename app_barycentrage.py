...
st.set_page_config(
    page_title="Simulation entrepôts Leclerc",
    page_icon="🏬",
    layout="wide",
)
...
DELIVERY_OPTIONS = ("Livraison centrale", "Livraison magasin")
WEIGHT_COL = "Poids - CA"
SCA_COL = "Libelle SCA"
STORE_ID_COL = "Identifiant fictif"

# Coordonnées manuelles des centrales, basées sur l'onglet recap.
CENTRALS = {
    "LECASUD": {
    ...
    "SCADIF": {
        "label": "SCADIF - Réau",
        "address": "Parc d'activité de l'A5, 77750 Réau",
        "lat": 48.5883,
        "lon": 2.6274,
    },
    "SCALANDES": {
        "label": "SCALANDES - Mont-de-Marsan",
        "address": "ZA de Pémégnan, 40001 Mont-de-Marsan",
        "lat": 43.8921,
        "lon": -0.5000,
    },
    "SCANORMANDE": {
    ...
    "SCAPARTOIS": {
        "label": "SCAPARTOIS - Tilloy-lès-Mofflaines",
        "address": "ZI Arras Est, 62217 Tilloy-lès-Mofflaines",
        "lat": 50.2827,
        "lon": 2.8223,
    },
    "SCAPEST": {
        "label": "SCAPEST - Saint-Martin-sur-le-Pré",
        "address": "ZI du Moulin, 51039 Châlons-en-Champagne",
        "lat": 48.9724,
        "lon": 4.3630,
    },
    "SCAPNOR": {
        "label": "SCAPNOR - Bruyères-sur-Oise",
        "address": "ZAE Chemin du bac des Aubins, 95820 Bruyères-sur-Oise",
        "lat": 49.1540,
        "lon": 2.3264,
    },
    "SCARMOR": {
    ...
    "SOCAMAINE": {
        "label": "SOCAMAINE - Champagné",
        "address": "Route de Paris, 72470 Champagné",
        "lat": 48.0224,
        "lon": 0.3320,
    },
    "SOCAMIL": {
        "label": "SOCAMIL - Castelnaudary",
        "address": "511 Av. Gérard Rouvière, 11400 Castelnaudary",
        "lat": 43.3189,
        "lon": 1.9539,
    },
    "SOCARA": {
...
def load_store_data() -> Tuple[pd.DataFrame, Path]:
    ...
    filtered["Nom affichage"] = filtered["Raison sociale"].fillna(
        filtered["Ville"].astype(str)
    )
...
def load_recap_data() -> Tuple[pd.DataFrame, Path]:
    path = find_excel_path("liste entrepôts")
...
def build_demand_points(stores_df: pd.DataFrame, delivery_modes: Dict[str, str]) -> pd.DataFrame:
    ...
    for sca, group in stores_df.groupby(SCA_COL, sort=True):
        mode = delivery_modes.get(sca, "Livraison centrale")
        if mode == "Livraison centrale":
    ...
    return points_df.sort_values("point_id").reset_index(drop=True)
...
def solve_north_assignment(
    ...
    for idx, store_count in enumerate(cum_store_count):
        ...
        elif int(store_count) < north_target:
            candidate_prefixes.add(idx + 1)
        elif int(store_count) > north_target:
            candidate_prefixes.add(idx + 1)
...
def assign_points(points_df: pd.DataFrame, south_warehouse: Dict[str, float | str], north_ratio_pct: float) -> pd.DataFrame:
    total_stores = int(points_df["store_count"].sum())
...
def compute_barycentric_solution(points_df: pd.DataFrame, north_ratio_pct: float) -> Tuple[pd.DataFrame, Dict[str, float | str]]:
    ...
    for _ in range(30):
        ...
        south = {
            "name": "Entrepôt Sud - Barycentre mathématique",
            "lat": new_lat,
            "lon": new_lon,
        }
...
def summarize_results(points_df: pd.DataFrame, stores_df: pd.DataFrame, south_warehouse: Dict[str, float | str]) -> SimulationResult:
    ...
    export_df["Distance km vers entrepot attribue"] = export_df["row_id"].map(
        {
            int(row_id): float(row.distance_km)
            for row in points_df.itertuples(index=False)
            for row_id in row.store_row_ids
        }
    )
    ...
    warehouse_summary = (
        ...
        .agg(
            nb_magasins=(STORE_ID_COL, "count"),
            poids_total=(WEIGHT_COL, "sum"),
        )
        .reset_index()
        .rename(columns={"Attribution entrepot": "Entrepôt"})
    )

    distances_summary = (
        points_df.groupby("assigned_warehouse")
        .agg(
            distance_ponderee=("weighted_distance", "sum"),
            distance_moyenne_km=("distance_km", "mean"),
        )
        .reset_index()
        .rename(columns={"assigned_warehouse": "Entrepôt"})
    )

    total_weighted_distance = float(points_df["weighted_distance"].sum())
...
def build_map_df(result: SimulationResult) -> pd.DataFrame:
    ...
    points["tooltip_name"] = points["display_name"]
    points["warehouse_label"] = points["assigned_warehouse"]
    points["poids_display"] = points[WEIGHT_COL].map(lambda x: f"{float(x):.6f}")

    warehouse_rows = [
        {
            "tooltip_name": NORD_WAREHOUSE["name"],
            "display_type": "Entrepôt",
            "warehouse_label": "Nord",
            "Latitude": NORD_WAREHOUSE["lat"],
            "Longitude": NORD_WAREHOUSE["lon"],
            WEIGHT_COL: points.loc[points["assigned_warehouse"] == "Nord", WEIGHT_COL].sum(),
            "poids_display": f"{points.loc[points['assigned_warehouse'] == 'Nord', WEIGHT_COL].sum():.6f}",
            "store_count": int(points.loc[points["assigned_warehouse"] == "Nord", "store_count"].sum()),
            "color": [8, 81, 156, 240],
            "height": max(points.loc[points["assigned_warehouse"] == "Nord", WEIGHT_COL].sum() * 900000, 5000),
        },
        {
            ...
            WEIGHT_COL: points.loc[points["assigned_warehouse"] == "Sud", WEIGHT_COL].sum(),
            "poids_display": f"{points.loc[points['assigned_warehouse'] == 'Sud', WEIGHT_COL].sum():.6f}",
            "store_count": int(points.loc[points["assigned_warehouse"] == "Sud", "store_count"].sum()),
            "color": [178, 56, 39, 240],
            "height": max(points.loc[points["assigned_warehouse"] == "Sud", WEIGHT_COL].sum() * 900000, 5000),
        },
    ]
...
def run_simulation(
    stores_df: pd.DataFrame, south_choice: str, north_ratio_pct: float, delivery_modes: Dict[str, str]
) -> SimulationResult:
    ...
    if normalized_choice.startswith("barycentrage"):
        ...
        matched_key = next(
            (key for key in FIXED_SOUTH_OPTIONS if key.lower() == normalized_choice),
            south_choice,
        )
...
def main() -> None:
    st.title("Simulation transport entrepôts Leclerc")
    st.caption(
    ...
    missing_centrals = sorted(set(sca_list) - set(CENTRALS))
    if missing_centrals:
        st.error(f"Coordonnées de centrales manquantes pour : {', '.join(missing_centrals)}")
        st.stop()

    with st.expander("Sources de données et hypothèses", expanded=False):
        st.write(f"Fichier magasins : `{stores_path.name}`")
        st.write(f"Fichier centrales : `{recap_path.name}`")
        st.write(f"Nombre de magasins simulés après exclusions : `{len(stores_df)}`")
        st.write(
            "Les coordonnées des centrales sont renseignées dans le code à partir de l'onglet "
    ...
    with st.form("simulation_form"):
        ...
        with left:
            south_choice = st.selectbox(
                "Localisation de l’entrepôt Sud",
                options=["Toulouse", "Avignon", "Barycentrage mathématique"],
                index=0,
            )
            north_ratio_pct = st.slider(
                "Pourcentage cible de magasins rattachés à l’entrepôt Nord",
                min_value=0,
                max_value=100,
                value=50,
                step=1,
        ...
        with right:
            st.markdown("**Mode de livraison par SCA régionale**")
    ...
    if not launch:
        st.info("Sélectionnez les paramètres puis cliquez sur `Lancer la simulation`.")
        return

    result = run_simulation(stores_df, south_choice, north_ratio_pct, delivery_modes)

    kpi_1, kpi_2, kpi_3 = st.columns(3)
    kpi_1.metric("Distance pondérée totale", f"{result.total_weighted_distance:,.2f}".replace(",", " "))
    kpi_2.metric(
        "Magasins rattachés au Nord",
        str(int(result.warehouse_summary.loc[result.warehouse_summary["Entrepôt"] == "Nord", "nb_magasins"].sum())),
    )
    kpi_3.metric(
        "Position Sud simulée",
    ...
    with info_left:
        st.subheader("Synthèse par entrepôt")
    ...
    with info_right:
        st.subheader("Distances pondérées")
    ...
    st.subheader("Points simulés")
    display_points = result.points_df[
        [
            "display_name",
            "display_type",
            SCA_COL,
    ...
    st.download_button(
        "Exporter le fichier Excel avec l’attribution entrepôt",
        data=export_bytes,
