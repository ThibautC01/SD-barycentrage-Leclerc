...
CENTRAL_FIXED_COST_PER_DELIVERY = 29.254
CENTRAL_VARIABLE_COST_PER_KM = 0.0348
...
# ---------------------------------------------------------------------------
# Mode applique aux SCA non listees ci-dessous // livraison aux magasins par défaut, répartition mathématique
...
def validate_configuration() -> None:
    configured = CENTRAL_DELIVERY_SCAS + SCA_BARYCENTER_DELIVERY_SCAS
...
def transport_cost(distance_km: np.ndarray, weights: np.ndarray, delivery_mode: str) -> np.ndarray:
    if delivery_mode == DELIVERY_CENTRAL:
        fixed_cost = CENTRAL_FIXED_COST_PER_DELIVERY
        variable_cost = CENTRAL_VARIABLE_COST_PER_KM
    else:
        fixed_cost = STORE_FIXED_COST_PER_DELIVERY
        variable_cost = STORE_VARIABLE_COST_PER_KM

    volume = weights * TOTAL_VOLUME
    return (fixed_cost + variable_cost * distance_km) * volume


def transport_cost_by_mode(
    distance_km: np.ndarray,
    weights: np.ndarray,
    delivery_modes: pd.Series,
) -> np.ndarray:
    costs = np.zeros(len(distance_km), dtype=float)
    central_mask = delivery_modes.eq(DELIVERY_CENTRAL).to_numpy()
    if central_mask.any():
        costs[central_mask] = transport_cost(
            distance_km[central_mask],
            weights[central_mask],
            DELIVERY_CENTRAL,
        )
    store_mask = ~central_mask
    if store_mask.any():
        costs[store_mask] = transport_cost(
            distance_km[store_mask],
            weights[store_mask],
            DELIVERY_STORE,
        )
    return costs
...
def assign_points(points_df: pd.DataFrame, south_warehouse: Warehouse, north_ratio_pct: int) -> pd.DataFrame:
    ...
    north_cost = transport_cost_by_mode(north_dist, weights, points_df["delivery_mode"])
    south_cost = transport_cost_by_mode(south_dist, weights, points_df["delivery_mode"])
